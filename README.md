...
| ---------------------- | ------ | ---------------- | -------------- | ------- | -------------- |
| EXAONE 3.5 3.5B        | 약 3.5B | 중상 *(경험 기반)*     | 보통 *(8bit 기준)* | 중간      | 지시문 최적화, 경량 버전 |
| KLUE-RoBERTa           | 약 110M | 중간 *(공식 보고서 기준)* | 빠름             | 낮음      | 분류형 QA 적합      |
| KoreALBERT (KcELECTRA) | 약 13M  | 낮음\~중간 *(경량 모델)* | 매우 빠름          | 매우 낮음   | 모바일/경량 환경 적합   |

## ✅ 변경 내용 요약표
| 항목      | 변경 전 (7.8B)               | 변경 후 (3.5B)          | 비고                    |
| ------- | ------------------------- | -------------------- | --------------------- |
| 모델 이름   | EXAONE 3.5 7.8B           | EXAONE 3.5 3.5B      | 동일 계열의 경량 버전 사용       |
| 파라미터 수  | 약 7.8B                    | 약 3.5B               | 약 55% 감축              |
| 로딩 속도   | 느림 *(checkpoints 단계)*     | 보통 *(8bit 양자화 기준)*   | 초기 로딩 병목 완화           |
| 응답 속도   | 느림                        | 체감 속도 개선             | 실시간 처리 가능성 증가         |
| 메모리 사용량 | 높음                        | 중간                   | GPU 적합도 상승            |
| 정확도 체감  | 높음 *(경험 기반)*              | 중상 *(거의 유사)*         | 큰 성능 저하 없음            |
| 사용 코드   | `AutoModelForCausalLM` 사용 | 동일 (`bnb_config` 적용) | BitsAndBytesConfig 유지 |
